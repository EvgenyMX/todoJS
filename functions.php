...
// registation CSS and JS
add_action( 'wp_enqueue_scripts', 'gp_styles' );
add_action( 'wp_enqueue_scripts', 'gp_js' );
function gp_styles() {
...
function gp_js() {
    ...
    wp_enqueue_script( 'responsiveslides', get_template_directory_uri() . '/assets/lib/rslides/responsiveslides.min.js', array('jquery'), null, true);
...
// Supporting title tag via add_theme_support
...
function add_custom_logo_url() {
    $html = sprintf( '<a href="%1$s" class="navbar-brand custom-logo-link">%2$s</a>',
            esc_url( '/' ),
            wp_get_attachment_image( get_theme_mod( 'custom_logo' ), array(205,275), false, array(
                'class'    => 'custom-logo',
            ) )
        );
    return $html;   
...
// size preview img product
add_image_size( 'preview_product', 199, 265, true );
add_image_size( 'product', 258, 345, true );


// Add new Post Type
add_action( 'init', 'grills' );
function grills() {
    $labels = array(
        'name'               => 'Мангалы',
        'singular_name'      => 'Мангал',
        'menu_name'          => 'Мангалы',
        'name_admin_bar'     => 'Мангал',
        'add_new'            => 'Добавить Мангал',
        'add_new_item'       => 'Добавить новую Мангал',
        'new_item'           => 'Новый Мангалы', 
        'edit_item'          => 'Редактировать Мангал',
        'view_item'          => 'Просмотр Мангала',
        'all_items'          => 'Мангалы',
        'search_items'       => 'Поиск Мангалы',
        'parent_item_colon'  => 'Родительская Мангала:', 
        'not_found'          => 'Не найдено ни одного Мангала.', 
        'not_found_in_trash' => 'В корзине не найдено ни одного Мангала.',
    );
    $args = array(
        'labels'             => $labels,
        'public'             => true,
    ...
    register_post_type( 'grills', $args );
}

// New Page to menu WP with ACFPro
...
// Код для определения сколько постов на странице должно выводиться в кастомных пунктах меню (кастомные посты)
function custom_post_per_page( $query ) {
    if( is_post_type_archive('price') ) {
        $query->set('posts_per_page', -1); //ваше значение
    }elseif (is_tax()){
        $query->set('posts_per_page',-1); //ваше значение
    }
    return $query;
...
// Custom Nav menu
class mainMenuGrillPoint extends Walker_Nav_Menu {
...
class footerMenuGrillPoint extends Walker_Nav_Menu {
	function start_lvl(&$output, $depth = 0, $args = array()) {
		$output .= '<ul class="menu_sublist">';
	}
	function start_el( &$output, $item, $depth = 0, $args = NULL, $id = 0 ) {
		...
		$attributes .= ! empty( $item->target )     ? ' target="' . esc_attr( $item->target     ) .'"' : '';
		$attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
		$attributes .= ! empty( $item->classes )        ? ' class="menu_item" ' : '';
		$attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
